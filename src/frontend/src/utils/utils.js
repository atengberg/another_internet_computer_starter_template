...
function prepareAmount(amount, decimals) {
  ...
  if (containsDecimalPoint(amountLiteral)) {
    ...
    if (!Number.isInteger(parseInt(amountLiteral))) {
      throw new Error(`Invalid amount ${amount} passed!`);
    }
...
function getSendPaymentArgs({
  ...
  return {
    ...
    amount: prepareAmount(amount, decimals)
  }
}
...
function prepareSendPaymentArgs({ 
  ...
  // before the backend canister call finishes and the
  // list is updated with most accurate info. 
